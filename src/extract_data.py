...
def is_user(line, current_group, current_access_level):
    ...
    if user_match:
        ...
        access_level_id = access_level_to_id(access_level)

        if "(" in user_match.group(0) and ")" in user_match.group(0):
            current_group = "SEM GRUPO ASSOCIADO"

        user_data[user] = [{'group': current_group, 'access_level': access_level_id}]
...
def access_level_to_id(access_level):
    access_level_map = {
        'N': 1,
        'R': 2,
        'RX': 3,
    ...
    return access_level_map.get(access_level, 3)
...
def read_task(spark):
    ...
    data_103 = spark.read.text(os.path.join(pasta_info, s103))
    data_207 = spark.read.text(os.path.join(pasta_info, s207))

    rdd_103_extracted = data_103.rdd.mapPartitions(process_lines)
    rdd_207_extracted = data_207.rdd.mapPartitions(process_lines)

    df_grouped_103 = rdd_103_extracted.toDF()
    df_grouped_207 = rdd_207_extracted.toDF()

    return df_grouped_103, df_grouped_207


def organize_task(df):
    df = df.select(
        "Folder",
        "Date",
        F.explode("Users").alias("User", "GroupsAccess"),
    )
    ...
    df = df.select(
        ...
        "Date",
        "User",
        F.explode("GroupsAccess").alias("GroupAccess"),
    )
